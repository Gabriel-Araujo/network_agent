...
path — downstream steps consume this file, not loose text in the
conversation.
` +
	"```markdown\n" +
	`# Intent Analysis — FRR
...
6. If stuck in Exstart/Exchange, check for an MTU mismatch between the
   ...
   **File produced** ('./.agent/tmp/network_agent/meu-ospf-nao-converge-entre-60becac1.md'):` +
	"```markdown" + `
   # Intent Analysis — FRR
